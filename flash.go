...
func WriteWindowsISOToBlockDevice(
	...
	if err != nil {
		return fmt.Errorf("failed to read UDF filesystem on ISO: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to get info about destination: %w", err)
	...
	if err != nil {
		return fmt.Errorf("failed to get size of destination: %w", err)
	...
	if err != nil && err != imaging.ErrNotBlockDevice { // Ignore non-block-device error here
		return fmt.Errorf("failed to unmount destination device: %w", err)
	}
	if ctx.Err() != nil {
		return fmt.Errorf("operation cancelled")
	}
	...
	if fs != "fat32" {
		...
		if err != nil {
			return fmt.Errorf("failed to write UEFI bootloader to second partition: %w", err)
		}
		if ctx.Err() != nil {
			return fmt.Errorf("operation cancelled")
		}
	...
	case "ntfs":
		if err := MakeNTFS(primaryPartition, sanitizeNTFSLabel(windowsVolumeLabel)); err != nil {
			return fmt.Errorf("failed to create NTFS filesystem: %w", err)
		}
	case "fat32":
	...
	if err = func() error {
		...
		if err := ValidateISOAgainstLocation(ctx, logFn, iso, mountPoint); err != nil {
			return fmt.Errorf("failed to validate ISO contents: %w", err)
		}
		return nil
	}(); err != nil {
		return err
	...
	if !gpt {
		...
		if err := WriteVBRToPartition(primaryPartition); err != nil {
			return fmt.Errorf("failed to write VBR bootloader: %w", err)
		}
		if err := WriteMBRToDisk(blockDevice); err != nil {
			return fmt.Errorf("failed to write MBR bootloader: %w", err)
		}
	}
